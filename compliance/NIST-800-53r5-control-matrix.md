...
Scope:
- Workload: AWS API Gateway + AWS Lambda + AWS Secrets Manager + CloudWatch Logs + KMS + AWS Config + Security Hub
...
| RA-5 Vulnerability Monitoring | Security Hub enabled with AWS Foundational Security Best Practices standard | `aws securityhub describe-hub`, `aws securityhub get-enabled-standards` + validation script checks | Shared |
